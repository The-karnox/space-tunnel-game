...
function animate(t = 0) {
  ...
  tubeMaterial.color.setHSL(hue, 1, 0.5);
  lineMat.color.setHSL(hue, 1, 0.5); 
...
audioLoader.load('assets/background.mp3', function(buffer) {
  ...
  bgReady = true; // Mark music as ready to play
});
...
function startBgMusic() {
  if (!bgStarted && bgReady) {
    bgSound.play();
    bgStarted = true;
    if (introReady) {
      introSound.play();
    }
    goFullscreen(); // Trigger fullscreen on first click
  }
}
...
audioLoader.load('assets/fire.mp3', function(buffer) {
